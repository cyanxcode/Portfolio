...
<nav class={`${isOpen ? 'top-0': '-top-[3rem]'} absolute flex flex-col w-full z-30 items-center transition-all duration-100 ease-in-out`}>
    <div class="w-full h-12 bg-white flex items-center justify-evenly font-medium text-base sm:text-lg">
        ...
        <a target="_blank" href="https://dribbble.com/CyanStudios" class="flex gap-2 items-center"><img src="/icons/dribbble.svg" alt="" class="w-6 md:w-8"><p class="hidden md:block">Dribbble </p></a>
        <a target="_blank" href="https://x.com/CyanStudios" class="flex gap-2 items-center"><img src="/icons/x.svg" alt="" class="w-6 md:w-8"><p class="hidden md:block">x.com </p></a>
    </div>
    <button onclick={() => isOpen = !isOpen} class="poly z-30  cursor-pointer">
